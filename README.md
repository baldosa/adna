...
classDiagram
    class Juego {
        ...
        +direccion: int
        +cartas_acumuladas: int
        +accion_pendiente: string
        +iniciar_juego()
        +jugar_ronda()
        +procesar_accion(Carta)
        +avanzar_turno()
        +es_jugada_valida(Carta, Carta) bool
        +verificar_ganador(Jugador) bool
    }
    class Mazo {
        -cartas: list~Carta~
        -pozo: list~Carta~
    ...
    class Jugador {
        ...
        +dijo_adna: bool
        +jugar(Juego) Carta
        +tomar_cartas_del_mazo(Mazo, int)
        +jugar_carta(int, Mazo)
        +decir_adna()
    }
    class JugadorHumano {
        +es_humano: bool
    ...
    Juego o-- "1" Mazo : "controla"
    Juego o-- "4" Jugador : "maneja"
    Mazo o-- "many" Carta : "contiene"
    Jugador o-- "many" Carta : "tiene (mano)"
